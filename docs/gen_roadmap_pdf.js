...
const FASE_A = [
  ...
  { id: "A6", name: "Cumbre del runtime  🚧 en progreso", hz: "C",
    exito: "**parcial** — **verificador JVMS-estricto** y **GC generacional** logrados; faltan la ejecución concurrente y/o el JIT.",
    items: [
      ...
      { hz: "C", txt: "**Hilos / concurrencia** (*en progreso*) — objetivo: **multithread profesional**, por fases. **✅ Fase 0 — green threads**: `java.lang.Thread` + `start`/`join`/`sleep`, scheduler cooperativo round-robin en el safepoint, varios call stacks por-thread, GC sobre las raíces de **todos** los threads, terminación; verificado (`Threads.run → 100`) y visible en el step-visualizer. **✅ Monitores**: `synchronized` de **bloques y métodos** (`ACC_SYNCHRONIZED`, sin opcode — el VM toma/suelta el monitor al entrar/salir del frame), reentrancia, y señalización `wait`/`notify`/`notifyAll` sobre *wait-set*/*blocked-set*, más `join`/`sleep`. El paralelismo de cómputo del LLM vive en **kernels off-heap** (rayon/CUDA), no en el heap Java. *La hoja de ruta de lo que falta para concurrencia profesional (OS threads, GIL, JMM, `java.util.concurrent`) tiene su propia sección («Concurrencia»).*" },
...
const TBL_CONCURRENCIA = [
  ...
  ["3 · API de `Thread`", "🟡 parcial", "**Hecho:** `start`/`run`/`join`/`sleep`. **Falta:** `interrupt`/`InterruptedException`, `yield`, `currentThread`, prioridades, daemon, estados completos."],
  ["2 · Monitor intrínseco", "🟡 parcial", "**Hecho:** `monitorenter`/`exit`, `synchronized` (bloques **y** métodos), reentrancia, `wait`/`notify`/`notifyAll`. **Falta:** `IllegalMonitorStateException`, `wait(timeout)`, monitor *GC-safe*, biased/thin locks."],
  ["1 · Substrato de ejecución", "🟡 parcial", "**Hecho:** green threads (scheduler cooperativo, 1 *carrier*). **Falta:** `park`/`unpark`, GIL, hilos de SO sin GIL (paralelismo real), *safepoints* stop-the-world, preempción."],
];
...
const TOWER_ASCII = [
  ...
  cBar("├", "┤"),
  cRow("[3]  API de Thread                    ·  control de hilos    PARCIAL"),
  cRow("     [x] start · run · join · sleep"),
  cRow("     [ ] interrupt · yield · currentThread · prioridad · daemon"),
  cBar("├", "┤"),
  cRow("[2]  Monitor intrínseco               ·  ← ACÁ ESTAMOS       PARCIAL"),
  cRow("     [x] monitorenter/exit · synchronized (bloques Y métodos)"),
  cRow("     [x] reentrancia · wait / notify / notifyAll"),
  cRow("     [ ] IllegalMonitorStateException · wait(timeout) · GC-safe"),
  cBar("├", "┤"),
  cRow("[1]  Substrato de ejecución           ·  paralelismo real    PARCIAL"),
  cRow("     [x] green threads (cooperativo, 1 carrier OS)"),
  cRow("     [ ] park/unpark · GIL · hilos de SO sin GIL · stop-the-world"),
  cBar("└", "┘"),
  "                     ▲ todo descansa en la planta 1 ▲",
].join("\n");

const ROUTE_ASCII = [
  "  HOY           cerrar monitor        cambiar substrato         desbloquear lo \"pro\"",
  " green          IMSE · wait(t)        GIL  →  SO sin GIL         JMM → CAS → AQS",
  " threads  ──►   · interrupt     ──►   (park/unpark reales) ──►   → java.util.concurrent",
].join("\n");
...
<p>De todo el edificio, <strong>dos primitivas</strong> sostienen el resto: <strong><code>park</code>/<code>unpark</code></strong> (planta 1) y <strong>CAS</strong> (planta 5). De esas dos se deduce <em>casi todo</em> lo de arriba — AQS, los locks, los atómicos, los pools. Por eso el salto grande <strong>no</strong> es terminar el monitor (planta 2, ya casi lista), sino cambiar el <strong>substrato</strong> de green threads a hilos de SO.</p>
...
<p class="exito"><strong>✓ Siguiente:</strong> seguir la concurrencia hacia el <strong>multithread profesional</strong> (ver la sección «Concurrencia»): cerrar el monitor (<code>IllegalMonitorStateException</code>, <code>wait(timeout)</code>, interrupts) y dar el salto de substrato — OS threads + GIL (park/unpark real) → sacar el GIL (TLABs + GC stop-the-world → Java en paralelo) → modelo de memoria (<code>volatile</code>/fences) → <code>java.util.concurrent</code>. El paralelismo de cómputo del LLM vive en <strong>kernels off-heap</strong> (rayon/CUDA).</p>
</div>

</body>
</html>`;

const outHtml = "D:/Proyectos/Rust/jvm/docs/Roadmap_JDK.html";
