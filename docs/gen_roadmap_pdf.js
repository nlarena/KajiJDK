...
const FASE_A = [
  ...
  { id: "A6", name: "Cumbre del runtime  🚧 en progreso", hz: "C",
    exito: "**parcial** — **verificador JVMS-estricto**, **GC generacional** e **hilos de SO reales tras un GIL** logrados; falta el **paralelismo real** (sacar el GIL), el JMM y/o el JIT.",
    items: [
...
const TBL_CONCURRENCIA = [
  ...
  ["3 · API de `Thread`", "🟡 parcial — **← acá (H1)**", "**Hecho:** `start`/`run`/`join`/`sleep` (ya *wall-clock* en modo OS). **Falta:** `interrupt`/`InterruptedException`, `yield`, `currentThread`, `Thread(Runnable)`, nombre/id, `isAlive`, `getState`, prioridades, daemon. Hoy `ThreadStatus` tiene cuatro estados (`Runnable`/`Blocked`/`Waiting`/`Terminated`): faltan `NEW` y `TIMED_WAITING` — este último modelado con el campo aparte `sleep_until`."],
...
const TOWER_ASCII = [
  ...
  cBar("├", "┤"),
  cRow("[3]  API de Thread                    ·  ← ACÁ ESTAMOS (H1)  PARCIAL"),
  cRow("     [x] start · run · join · sleep (wall-clock en modo OS)"),
  cRow("     [ ] interrupt · yield · currentThread · getState · daemon"),
  cBar("├", "┤"),
  cRow("[2]  Monitor intrínseco               ·  monitor completo       CASI"),
  cRow("     [x] monitorenter/exit · synchronized (bloques Y métodos)"),
  cRow("     [x] reentrancia · wait / notify / notifyAll · IMSE"),
  cRow("     [x] wait(timeout) · GC-safe (sobrevive minor/compact)"),
  cRow("     [ ] interrupt-of-wait (→ H1) · biased/thin locks"),
  cBar("├", "┤"),
  cRow("[1]  Substrato de ejecución           ·  paralelismo real    PARCIAL"),
...
const ROUTE_ASCII = [
  "  ✔ HECHO             ✔ HECHO             ◀ ACÁ (H1)          DESPUÉS (E3 → pro)",
...
<p class="exito"><strong>✓ Siguiente:</strong> <strong>H1 — la API de <code>Thread</code></strong> (ver la sección «Concurrencia»): extender <code>ThreadStatus</code> a los seis estados de <code>Thread.State</code>, sumar <code>currentThread</code>/<code>yield</code>/<code>getState</code>/<code>isAlive</code> y <code>Thread(Runnable)</code>, y cerrar <code>interrupt</code>/<code>InterruptedException</code> — que de paso completa la última pieza del monitor. Después, <strong>E3</strong>: sacar el GIL (locks finos + TLABs + handshake stop-the-world → Java en paralelo) → modelo de memoria (<code>volatile</code>/fences) → CAS → <code>java.util.concurrent</code>. El paralelismo de cómputo del LLM vive en <strong>kernels off-heap</strong> (rayon/CUDA).</p>
</div>
