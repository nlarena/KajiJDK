...
- [ ] **`java.lang.ref` restante**: `SoftReference` (política de presión de GC), `PhantomReference`, `Cleaner`. `finalize()` **no se implementa por decisión** (deprecado-for-removal en el JDK real)
- [ ] **Ciclo de vida**: `System.exit(int)`/`Runtime` (lo pedirá el lanzador `java`, Fase D); regla fina de init de interfaces (§5.5, verificar junto con default methods); periféricos de `Thread` (`ThreadGroup`, `sleep(long,int)`, `onSpinWait`, `setUncaughtExceptionHandler`)
