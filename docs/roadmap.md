...
- [ ] **Pendiente, por impacto:** el techo ya no son las llamadas sino el **merge del mapa de tipos** — un slot que llega `int` por una arista y referencia por otra (la forma habitual de javac cuando un slot está muerto en ese punto) descalifica el método entero; es lo que todavía deja afuera a `BmField`. Después: **registros** para la pila de operandos; y habilitar el JIT en **`os-parallel`** — apagado a propósito: es el único modo con el heisenbug abierto y no se le agrega una variable a un problema con ~17 sesiones de acotamiento
