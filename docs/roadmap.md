...
- [ ] **Que el store de cada worker se llegue a observar** — lo primero que dijo la medida: **53% del conjunto visitado**, y el histograma `#0x1 #1x19 #2x53 #3x327` sobre 400 corridas. Gana el último en arrancar 82% de las veces y el primero fue observado **una sola vez**, o sea que su store está sin probar. La causa está a la vista en `emit_race_worker`: el `run()` es un único store, y arrancar un thread cuesta órdenes de magnitud más que eso, así que cada worker termina antes de que el siguiente exista y el orden de llegada es el orden de arranque. Subir `repeats` no lo arregla —con `p₀ ≈ 0,0025` hacen falta ~400 corridas para ver al worker 0 *una vez*—; lo que hay que cambiar es la forma. **Listo cuando** ninguna posición del conjunto quede por debajo de un piso de victorias que no dependa del orden de arranque
