...
- [ ] **`ldc` de String** (46) — bloqueado por una carencia **de la VM, no del JIT**: `strings::intern` no tiene tabla de interning, hace un `malloc` fresco en Eden en cada ejecución. Dos consecuencias: hornear el offset de un literal sería corrupción (el objeto no está pinneado y el colector lo mueve), y —más grave— **`"a" == "a"` da `false` en esta VM**, que es una no-conformidad con la JLS §3.10.5 existente con o sin JIT. Arreglar el interning es una tarea de conformidad; que además destrabe 46 métodos del compilador es una consecuencia, no su motivo
...
### Hito K2 · Ensanchado de la gramática 🟣 — 🚧 3 de 5 etapas
...
- [ ] **Etapa 4 · tipos angostos (`byte`/`short`/`char`) y `switch`** — las conversiones estrechantes son fuente real de bugs, pero multiplican el contexto de tipos
- [ ] **Etapa 5 · `while`/`break`/`continue` y `throw` explícito** — ninguno se acota estructuralmente como un `for` contado; necesitan otro argumento de terminación
- [ ] **Strings — desbloqueado.** Estaban afuera porque `strings::intern` estaba *en obra*: generarlos habría fabricado divergencias que nadie podía clasificar. La tabla de interning aterrizó (F3, hito 3), así que es lo primero que corresponde agregar
- [ ] **`invokeinterface`** — comparte el camino de la caché de inline con `invokevirtual` pero resuelve por **itable, no vtable**: es una búsqueda genuinamente distinta. Necesita un local de tipo interfaz, o sea un segundo tipo estático en el `Scope`
- [ ] **Campos de referencia** — un `putfield` de referencia necesita la **barrera de escritura** del GC, que este tier del JIT contesta `Ineligible`. Todo método que tocara uno saldría del brazo compilado: FZ-004 con el disfraz de la barrera. Es lo primero que entra cuando la barrera aterrice
- [ ] **Hilos** — la razón misma por la que FZ-002 quiere un fuzzer, y un nivel propio
...
- [ ] **FZ-001** (la VM no expone la clase de la excepción por `run-headless`) y **FZ-002** (`DANGLING` intermitente en `verify_heap`, 1 de 10 corridas) siguen abiertos

**Estado (2026-08-28):** 120 semillas × 4 pareos (intérprete/JIT, VM/JDK real, green/os-gil, green/os-parallel): **100% usable, 0 divergencias**. Más 200 semillas en proceso, 161 entrando a código nativo. Ningún bug de conformidad encontrado todavía — lo que ese cero mide es lo que la gramática alcanza hoy, no la VM.
