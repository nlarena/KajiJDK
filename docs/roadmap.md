...
- [ ] **Minimizar una forma que hoy resiste la reducción** — es greedy, así que llega a un mínimo **local**, y bajo el oráculo de repetición su predicado además es inestable. **El caso, que ya apareció**: encoge las **hojas** de una condición pero no su **forma**. Con el `>>>` que el predicado necesita metido en la guarda de un `while`, deja todas las hojas en `0.0` y conserva el `&&` y el `||` que las juntan, cuando quedarse con una sola rama alcanzaría — 24 nodos donde entrarían menos. **Listo cuando** una forma que hoy resiste la reducción se minimiza, con el caso anotado
