...
- [x] **Strings — hecho (2026-08-29)** — y la etapa encontró un bug de conformidad de la VM, que es lo que un fuzzer diferencial existe para hacer. Estaban afuera porque `strings::intern` estaba *en obra*; **la afirmación de que la tabla de interning había aterrizado era falsa** y se comprobó midiendo: `"a" == "a"` sigue dando `false` acá y `true` en un JDK real ([FZ-008](fuzzer_findings/FZ-008-literales-sin-internar.md), abierto). La campaña no lo veía porque la sonda emitía la comparación **en línea** y `javac` la pliega antes de que la VM mire un `ldc` ([FZ-009](fuzzer_findings/FZ-009-la-sonda-que-el-compilador-borraba.md)): pasándola por parámetros de un método, la misma campaña da **2 divergencias en 40 semillas**
- [x] **`invokeinterface` (2026-08-29)** — resuelve por **itable, no vtable**: no hay slot estable porque cada interfaz numera sus métodos por su cuenta, así que la búsqueda es por firma. El `Local` gana el tipo **declarado** aparte del de la instancia, y declararlo como la interfaz es todo lo que hace falta: el opcode lo elige `javac`, no el generador. La restricción —por una interfaz solo se llega a `v()`, no a un campo ni a `w()`— se hace cumplir dejando esos nombres **fuera** de `Scope::objects`, así que cada sitio que ya existía sigue siendo correcto sin una guarda nueva. La sonda polimórfica plantada tiene ahora su variante por interfaz, que es el único lugar donde el receptor **cambia de clase en cada vuelta** de un bucle lo bastante caliente como para compilarse: la guarda del inline cache falla siempre en vez de asentarse. **Censo del class file** (que es lo único que prueba el opcode, porque quien lo elige es un compilador): **40 sitios en 21/60 semillas**, y **0** al sabotear la declaración. **No cuesta cobertura del JIT**, medido y no supuesto: sin interfaces 67/80 semillas entran a nativo con 364 métodos compilados, con ellas 71/80 y 370. **4 pareos × 300 semillas, 100% usables, 0 divergencias**
- [x] **Hilos — hecho (2026-08-29)** — dos piezas, y el detector primero porque un detector sin nada que detectar todavía se puede validar, y una gramática sin detector no.
  - **El oráculo que no necesita referencia** (`repetition_campaign`): el **mismo** programa N veces en `os-parallel`, comparado **contra sí mismo**. No sabe cuál es la respuesta correcta y no le hace falta: un programa que la gramática hace determinista contestando dos cosas distintas es un hallazgo, fuera cual fuera la buena. Un pareo es ciego a eso — tira la misma moneda de los dos lados —; la repetición convierte cada corrida extra en otra oportunidad, a costo lineal. El predicado del reductor recibe **el doble** de corridas que el detector: un shrinker sobre un predicado inestable que sea *menos* sensible que el detector da por sano al candidato que tuvo suerte, se queda con el corte, y devuelve un caso mínimo que no reproduce nada
  - **La gramática concurrente** (`Stmt::Fork`): K hilos, cada uno computando un `int` en **su propio** slot de un array, todos joineados, y reducidos en orden de índice. La forma es rígida y tiene que serlo, porque la propiedad 2 dice que un programa generado es determinista y la concurrencia es lo único acá que podría romperla: cada worker escribe **solo** su slot (no hay carrera de escritura que ordenar), todo `join` ocurre antes de **toda** lectura (nada se lee en vuelo), y la reducción recorre en orden de índice (**qué hilo terminó primero no se puede observar**). Una cuarta regla la hace comparable además de determinista: cada worker se atrapa a sí mismo, porque una excepción que escapa de un hilo es legal y determinista pero los dos lados la clasifican distinto — sería una divergencia fabricada por la herramienta
  - **Sabotaje**: emitiendo el `join` dentro de un `if (false)` —o sea desactivando una de las tres razones— la campaña encuentra la carrera en **3 de las primeras 4 semillas**, y nombra cuál corrida discrepó. No es un truco plantado: es una carrera de concurrencia real en Java generado, detectada sin ninguna implementación de referencia
  - **Cierre**: contra el JDK real **120 semillas, 100% usables, 0 divergencias**; contra sí mismo **120 semillas × 20 corridas = 2400 ejecuciones, 0 divergencias**
...
- [x] **FZ-001 — arreglado (2026-08-29)** — la VM siempre supo qué excepción mató al hilo: el reporte que construye `report_uncaught` iba al buffer de consola y `run-headless` nunca lo volcaba. La información existía y se perdía en el último metro. Ahora la clase se anota en el `SharedVm` —estructural, no buscando `Exception in thread` en un texto que un programa puede imprimir por su cuenta—, `execute_reporting` la saca junto con la consola, y el binario la vuelca a **stderr** (nunca a stdout: la línea de resultado la parsea el runner) saliendo con **1**, como `java`. Los dos lados se leen con **la misma** función
...
- [ ] **FZ-002 — perseguido con el instrumento, no reproducido (2026-08-29)** — el reproductor nombrado en el hallazgo da **60 de 60 en verde** con `JVM_GC_VERIFY=1`, y los programas concurrentes dan **0 divergencias en 4000 corridas** con recolección automática, promoción agresiva y el verificador puesto.
  - **La cifra que hace que ese cero signifique algo** no es el cero: es **528 colecciones por corrida de media** (máximo 2016). La primera versión de la campaña corría con la recolección automática apagada —es el default— y coleccionaba **2 veces**; un reporte limpio ahí habría parecido idéntico a éste y no habría dicho nada. FZ-004 aplicado por adelantado: primero se mide que el instrumento toque el sistema, y recién después significa algo que se calle. Segunda cifra que lo califica: **11 de 40 semillas mueren en un marcador** y trabajan muy poco
  - **La auditoría de raíces salió limpia**: frames, `parked_exception`, el pool de frames, los monitores (remapeados), condy, `thread_obj`/`wait_reacquire`, los mirrors (pinneados), el remembered set (reconstruido tras el major **y** tras compactar) y las referencias entre objetos (reescritas por `compact` sobre todos los objetos). Dos cosas salieron de ahí y ninguna es el bug: **una escritura de referencia que se salteaba la barrera** (`capture_backtrace` usaba `write_u32` en vez de la puerta única — hoy inofensiva porque el interning vive en Old, que es un hecho sobre la tabla y no sobre esa línea) y **un aviso viejo en `compact`** que decía que las referencias entre objetos quedaban sin reescribir. Eso dejó de ser cierto hace mucho, y un aviso que sobrevivió a su causa es peor que ninguno: **invita a quien persigue un puntero colgado a descartar ese camino por conocido-roto**
  - **No se le baja la severidad por no aparecer.** La evidencia original son 10 corridas con 1 aparición: poco para afirmar «1 de cada 10» y demasiado para descartarlo. Sigue abierto
  - **Lo que falta**: campos de **referencia** en la gramática, que es la arista exacta del síntoma (un campo de referencia de un objeto Old apuntando a Eden) y que sigue bloqueada por la barrera de escritura del JIT; y correr en otra máquina, porque el conteo de núcleos es parte del experimento
- [x] **FZ-008 — arreglado (2026-08-29)** — **el primer bug de conformidad de la VM que encontró el fuzzer**, y cerrado el mismo día. Los literales de `String` no se interneaban (JLS §3.10.5). El pool vive en el **metaspace** y no en el heap, que es lo que le da raíz y pinneado por la misma vía que ya usaban los mirrors; **alocado en Old**, porque un literal sobrevive a toda colección por definición. Las tres propiedades son independientes y ninguna es opcional: una instancia por literal, **raíz** (entre dos `ldc` del mismo literal no lo referencia nada más — eso *es* un pool) y **pinneado**, por una razón que los mirrors no comparten: `compact` reescribe todas las referencias, pero el pool está indexado por **contenido**, así que un literal movido deja el mapa nombrando la dirección vieja. Más la otra mitad de la regla: `intern` (pooleado, tres llamadores, todos literales) frente a `allocate` (fresco), porque un pool que se tragara los strings calculados haría `new String("a") == "a"` verdadero, que está igual de mal. **Sonda de cinco propiedades: 31 contra 31 del JDK real en los cuatro caminos y bajo presión de GC**; campañas 200×4 sin divergencias
- [x] **El sabotaje que no falló** — de tres roturas deliberadas, la de sacar el pool del conjunto `pinned` **pasó**: el test alocaba el hueco antes del primer literal y el mirror de `String`, que también está pinneado, lo tapaba. Nada se movía con pin o sin él. Reordenado para que el literal quede **detrás** del hueco, el sabotaje lo falla
...
- [x] **Campos de referencia — hecho (2026-08-29)** — un campo `c` de tipo referencia en la jerarquía, su `putfield` como sentencia y la lectura encadenada `o.c.a` como expresión. Es lo único de la gramática que construye una arista de un objeto del heap a otro, o sea lo que ejercita la **barrera de escritura** y el remembered set.
  - **El bloqueo anotado valía para un solo pareo**, y ahora está medido en vez de argumentado: contra el JIT los rechazos por método suben de **116 a 243**, así que parearlo ahí sería comparar el intérprete consigo mismo. Va con perilla propia, apagada por defecto, y las campañas lo usan contra el **JDK real** y contra **sí mismo en `os-parallel`** — igual que `narrowing_share`, que tira para lados opuestos por la misma razón
  - **La primera versión dejaba `c` en `null`** y el receptor nulo parecía un bonus: es un deopt gratis en el brazo compilado. Medido, era lo contrario — **41 de 80 semillas morían en un marcador** contra 9 del default, porque toda cadena lanzaba en la iteración 1 del calentamiento. FZ-005 otra vez. El constructor ahora lo inicializa a `this`, y el `null` se alcanza como todo lo demás en esta gramática: con una sentencia que lo pone ahí. **13 de 80** después del arreglo, y la entrada a nativo vuelve de 39/80 a **67/80**
  - **Sabotaje**: no generar la escritura, y declarar el campo siempre en vez de sólo cuando se usa. Los dos fallan el censo. Campañas: contra el JDK real **40 semillas, 0 divergencias**; contra sí mismo bajo GC automático con verificador, **40 × 10 corridas, 0 divergencias**
- [x] **Bajar las semillas que mueren en un marcador — hecho (2026-08-29)** — la causa era una línea de emisión: el envoltorio total atrapaba **alrededor del bucle** de calentamiento, así que una excepción en la iteración 1 terminaba el programa entero. Las iteraciones restantes no corrían, el JIT nunca cruzaba su umbral, y la campaña comparaba dos motores sobre un programa que no hizo nada — FZ-005 exacto.
  - **El arreglo**: el `catch` va **por iteración**. Una excepción pasa a ser un *valor* en el acumulador y el bucle sigue, así que un programa que lanza siempre igual devuelve algo que depende de todas las iteraciones, y uno que lanza una vez apenas se despeina. La propiedad 4 (totalidad) no cambia: los mismos marcadores, las mismas atrapadas
  - **Medido en las once configuraciones del censo**, no en una: las semillas que entran a código nativo pasan de **58–75 de 80 a 78–80 de 80**, y los métodos compilados suben en todas (el default, 370 → **406**; arrays sin floats, 358 → **441**; sin objetos, 99 → **120**). Las muertas en marcador van de 5–41 a **0 en todas**
  - **El número está en el reporte** (`Report::marked` y `marked_fraction`, impresos por cada campaña) **y la campaña larga falla por él** si pasa del 10%. Vive en el trait `Program`, porque sólo el generador sabe cuáles son sus marcadores
  - **Honestidad sobre la métrica**: después del arreglo lee 0 por construcción, porque con el acumulador de por medio un programa que lanza siempre devuelve `31·…·marca` y no la marca pelada. Lo que sigue atrapando exacto es el caso degenerado —el que no hace trabajo— y una regresión que devuelva el `try` afuera del bucle. **Sabotaje**: que el valor atrapado no llegue al acumulador falla el test
- [x] **`instanceof` y casts — hecho (2026-08-29)** — la misma pregunta hecha de las dos formas en que la hace la JVM: `instanceof`, que **nunca falla** (`null instanceof X` es `false` por JLS §15.20.2, no una `NullPointerException`), y el `checkcast`, que **sí**. En código compilado el cast que falla es un **deopt** en vez de un throw, así que los dos brazos llegan a la misma respuesta por caminos distintos.
  - **Encendido por defecto**, al revés que los otros dos de este hito, y por el mismo criterio que decide todas las perillas: `checkcast` e `instanceof` están **dentro** del subconjunto del JIT, así que suman cobertura en vez de restarla. Medido: el default queda en **79/80 semillas entrando a nativo y 398 métodos compilados**, contra 79/80 y 406 sin ellos
  - **Destrabó un marcador que estaba escrito y era inalcanzable**: `marks::CLASS_CAST` estaba en la tabla desde el principio y ningún nodo de la gramática podía producirlo. Ahora hay uno, y el envoltorio lo atrapa aparte — sin eso llegaría como `OTHER`, que es la misma respuesta que da un bug de cualquier otro constructo
  - **Van como sentencia y no como expresión**, por una razón del emisor y no de Java: nombrar la clase destino necesita el prefijo del programa, que `emit_stmt` tiene y `emit_expr` no. Pasarlo por 33 sitios de llamada para ahorrarse un local habría sido un cambio ancho por cero cobertura
  - **La sonda se sortea arriba de la rama del `new`**, porque no construye nada: sólo necesita un receptor que ya exista. Abajo llegaba al 10% de los programas con una perilla al 40 — el mismo error que había cometido el store de referencia. Y el listón del censo es **lo medido** (27/200), no lo deseado: el techo lo pone `object_share`, cuántas veces se sortea una sentencia de objetos, y no esta perilla
  - **Sabotajes**: generar sólo `instanceof` y nunca el cast, y quitar el `catch` de `ClassCastException`. Los dos fallan el censo. Campaña: **200 semillas × 4 pareos, 100% usables, 0% marcadores, 0 divergencias**
- [x] **El resto está afuera por decisión, no por deuda** — arrays multidimensionales, arrays `null`, payloads de NaN, locales `char`/`byte`/`short`/`boolean`, `do`/`while` y saltos etiquetados, y recursión. Cada uno con su razón escrita en la tabla de alcance del encabezado de `src/fuzz/gen.rs`, que es donde la lee quien va a tocar el generador. Un alcance no declarado es indistinguible de un descuido, y una lista de exclusiones sin argumentos es una forma elegante de esconder trabajo sin hacer
