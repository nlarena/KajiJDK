 ...
 * only that class can reinterpret its contents at other widths. A {@code IntBuffer} is either
 * allocated on its own — and then it owns a {@code int[]} — or obtained as a <em>view</em> over
 * some bytes via {@link ByteBuffer#asIntBuffer()}, and then it owns nothing and every access is
 ...
 * {@link #put(int, int)} rather than reaching into the array: a view has no array to reach into.
 *
...
public abstract class IntBuffer extends Buffer implements Comparable<IntBuffer> {
    ...
    IntBuffer(int[] hb, int offset, int capacity) {
        super(capacity);
        this.hb = hb;
        this.offset = offset;
    }
    ...
    public static IntBuffer allocate(int capacity) {
        ...
        return new HeapIntBuffer(new int[capacity], 0, capacity);
    }
    ...
    public static IntBuffer wrap(int[] array, int offset, int length) {
        HeapIntBuffer b = new HeapIntBuffer(array, 0, array.length);
        b.position(offset);
     ...
     * @throws ReadOnlyBufferException if this buffer is read-only
     */
    public abstract IntBuffer put(int index, int value);
     ...
     */
    public IntBuffer get(int index, int[] dst, int off, int length) {
    ...
    private IntBuffer getArray(int index, int[] dst, int off, int length) {
        int i = 0;
        while (i < length) {
     ...
     * @throws ReadOnlyBufferException if this buffer is read-only
     */
    public IntBuffer put(int index, int[] src, int off, int length) {
        checkWritable();
        checkIndex(index, length);
        Buffer.checkBounds(off, length, src.length);
    ...
    IntBuffer putArray(int index, int[] src, int off, int length) {
        int i = 0;
        while (i < length) {
     ...
     * @throws ReadOnlyBufferException if this buffer is read-only
     */
    public IntBuffer put(int index, IntBuffer src, int off, int length) {
        checkWritable();
        checkIndex(index, length);
    ...
    /** The unchecked copy behind {@link #put(int, IntBuffer, int, int)}. */
    void putBuffer(int index, IntBuffer src, int off, int length) {
        int i = 0;
        while (i < length) {
     ...
     *
     * <p><strong>Declared {@code Object} rather than {@code int[]}</strong>, unlike the JDK: our
     ...
     *
     * @return 2, because a int is four bytes wide
     */
    int scaleShifts() {
        return 2;
    }
     ...
     * @return {@code true} if {@code other} is a {@code IntBuffer} with the same
     *         remaining contents
     */
    public boolean equals(Object other) {
        boolean same = false;
        if (other instanceof IntBuffer) {
            IntBuffer that = (IntBuffer) other;
     ...
     *
     * @return a string of the form {@code IntBuffer[pos=.. lim=.. cap=..]}
     */
    public String toString() {
        return "IntBuffer[pos=" + position() + " lim=" + limit() + " cap=" + capacity() + "]";
    }
}

/**
 * The heap implementation of {@link IntBuffer}. Top-level and package-private rather than
...
class HeapIntBuffer extends IntBuffer {
    ...
    HeapIntBuffer(int[] hb, int offset, int capacity) {
    ...
    public IntBuffer slice() {
        HeapIntBuffer b = new HeapIntBuffer(hb, offset + position(), remaining());
    ...
    public IntBuffer slice(int index, int length) {
        checkIndex(index, length);
        HeapIntBuffer b = new HeapIntBuffer(hb, offset + index, length);
        b.isReadOnly = isReadOnly;
        return b;
    }

    public IntBuffer duplicate() {
        HeapIntBuffer b = new HeapIntBuffer(hb, offset, capacity());
        b.position(position());
        b.limit(limit());
        b.isReadOnly = isReadOnly;
        return b;
    ...
    public IntBuffer asReadOnlyBuffer() {
        HeapIntBuffer b = (HeapIntBuffer) duplicate();
        b.isReadOnly = true;
        return b;
    }
