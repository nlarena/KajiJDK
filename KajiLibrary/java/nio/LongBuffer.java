 ...
 * only that class can reinterpret its contents at other widths. A {@code LongBuffer} is either
 * allocated on its own — and then it owns a {@code long[]} — or obtained as a <em>view</em> over
 * some bytes via {@link ByteBuffer#asLongBuffer()}, and then it owns nothing and every access is
 ...
 * {@link #put(int, long)} rather than reaching into the array: a view has no array to reach into.
 *
...
public abstract class LongBuffer extends Buffer implements Comparable<LongBuffer> {
    ...
    LongBuffer(long[] hb, int offset, int capacity) {
        super(capacity);
        this.hb = hb;
        this.offset = offset;
    }
    ...
    public static LongBuffer allocate(int capacity) {
        ...
        return new HeapLongBuffer(new long[capacity], 0, capacity);
    }
    ...
    public static LongBuffer wrap(long[] array, int offset, int length) {
        HeapLongBuffer b = new HeapLongBuffer(array, 0, array.length);
        b.position(offset);
     ...
     */
    public abstract LongBuffer put(long value);
     ...
     * @throws ReadOnlyBufferException if this buffer is read-only
     */
    public abstract LongBuffer put(int index, long value);
    ...
    private LongBuffer getArray(int index, long[] dst, int off, int length) {
        int i = 0;
        while (i < length) {
    ...
    LongBuffer putArray(int index, long[] src, int off, int length) {
        int i = 0;
        while (i < length) {
     ...
     * @throws ReadOnlyBufferException if this buffer is read-only
     */
    public LongBuffer put(int index, LongBuffer src, int off, int length) {
        checkWritable();
        checkIndex(index, length);
    ...
    /** The unchecked copy behind {@link #put(int, LongBuffer, int, int)}. */
    void putBuffer(int index, LongBuffer src, int off, int length) {
        int i = 0;
        while (i < length) {
     ...
     *
     * @return 3, because a long is eight bytes wide
     */
    int scaleShifts() {
        return 3;
    }
     ...
     * @return {@code true} if {@code other} is a {@code LongBuffer} with the same
     *         remaining contents
     */
    public boolean equals(Object other) {
        boolean same = false;
        if (other instanceof LongBuffer) {
            LongBuffer that = (LongBuffer) other;
     ...
     *
     * @return a string of the form {@code LongBuffer[pos=.. lim=.. cap=..]}
     */
    public String toString() {
        return "LongBuffer[pos=" + position() + " lim=" + limit() + " cap=" + capacity() + "]";
    }
}

/**
 * The heap implementation of {@link LongBuffer}. Top-level and package-private rather than
...
class HeapLongBuffer extends LongBuffer {
    ...
    HeapLongBuffer(long[] hb, int offset, int capacity) {
    ...
    public LongBuffer slice() {
        HeapLongBuffer b = new HeapLongBuffer(hb, offset + position(), remaining());
    ...
    public LongBuffer slice(int index, int length) {
        checkIndex(index, length);
        HeapLongBuffer b = new HeapLongBuffer(hb, offset + index, length);
        b.isReadOnly = isReadOnly;
        return b;
    }

    public LongBuffer duplicate() {
        HeapLongBuffer b = new HeapLongBuffer(hb, offset, capacity());
        b.position(position());
        b.limit(limit());
        b.isReadOnly = isReadOnly;
        return b;
    ...
    public LongBuffer asReadOnlyBuffer() {
        HeapLongBuffer b = (HeapLongBuffer) duplicate();
        b.isReadOnly = true;
        return b;
    }
