...
/**
 * A {@link IntBuffer} that owns no array: every element is decoded from, and encoded into, the
 * bytes of a {@link ByteBuffer}. This is what {@link ByteBuffer#asIntBuffer()} returns.
 *
 ...
 * reinterpretation made into an object: {@code capacity} counts ints rather than bytes, the
 ...
 * {@code ByteBufferAsIntBufferB} / {@code ...L} / {@code ...RB} / {@code ...RL} quartet. Those
 ...
 */
class ByteViewIntBuffer extends IntBuffer {
    ...
    ByteViewIntBuffer(byte[] bytes, int byteOffset, int capacity, boolean bigEndian,
                      boolean isReadOnly) {
    ...
    private int decode(int at) {
        return (int) ByteCodec.read(bytes, at, 4, bigEndian);
    }

    /** Writes one element into the bytes starting at index {@code at}. */
    private void encode(int at, int value) {
        ByteCodec.write(bytes, at, 4, bigEndian, (long) value);
    }
    ...
    public IntBuffer slice() {
        return new ByteViewIntBuffer(bytes, byteOffset + 4 * position(), remaining(), bigEndian,
                isReadOnly);
    }

    public IntBuffer slice(int index, int length) {
        checkIndex(index, length);
        return new ByteViewIntBuffer(bytes, byteOffset + 4 * index, length, bigEndian,
                isReadOnly);
    }

    public IntBuffer duplicate() {
        ByteViewIntBuffer b =
                new ByteViewIntBuffer(bytes, byteOffset, capacity(), bigEndian, isReadOnly);
        b.position(position());
        b.limit(limit());
        return b;
    }
    ...
    public IntBuffer asReadOnlyBuffer() {
        ByteViewIntBuffer b =
                new ByteViewIntBuffer(bytes, byteOffset, capacity(), bigEndian, true);
        b.position(position());
        b.limit(limit());
        return b;
    }
