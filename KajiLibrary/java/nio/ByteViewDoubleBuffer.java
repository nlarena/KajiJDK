...
class ByteViewDoubleBuffer extends DoubleBuffer {
    ...
    ByteViewDoubleBuffer(ByteBuffer bytes, int byteOffset, int capacity, boolean bigEndian,
                      boolean isReadOnly) {
    ...
    private double decode(int at) {
        return ByteCodec.bitsToDouble(bytes.peek(at, 8, bigEndian));
    }

    /** Writes one element into the bytes starting at index {@code at}. */
    private void encode(int at, double value) {
        bytes.poke(at, 8, bigEndian, ByteCodec.doubleToBits(value));
    }
