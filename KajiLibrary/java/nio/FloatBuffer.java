 ...
 * only that class can reinterpret its contents at other widths. A {@code FloatBuffer} is either
 * allocated on its own — and then it owns a {@code float[]} — or obtained as a <em>view</em> over
 * some bytes via {@link ByteBuffer#asFloatBuffer()}, and then it owns nothing and every access is
 ...
 * {@link #put(int, float)} rather than reaching into the array: a view has no array to reach into.
 *
...
public abstract class FloatBuffer extends Buffer implements Comparable<FloatBuffer> {
    ...
    FloatBuffer(float[] hb, int offset, int capacity) {
        super(capacity);
        this.hb = hb;
        this.offset = offset;
    }
    ...
    public static FloatBuffer allocate(int capacity) {
        ...
        return new HeapFloatBuffer(new float[capacity], 0, capacity);
    }
    ...
    public static FloatBuffer wrap(float[] array, int offset, int length) {
        HeapFloatBuffer b = new HeapFloatBuffer(array, 0, array.length);
        b.position(offset);
     ...
     * @throws ReadOnlyBufferException if this buffer is read-only
     */
    public abstract FloatBuffer put(int index, float value);
     ...
     */
    public FloatBuffer get(int index, float[] dst, int off, int length) {
    ...
    private FloatBuffer getArray(int index, float[] dst, int off, int length) {
        int i = 0;
        while (i < length) {
     ...
     * @throws ReadOnlyBufferException if this buffer is read-only
     */
    public FloatBuffer put(int index, float[] src, int off, int length) {
        checkWritable();
        checkIndex(index, length);
        Buffer.checkBounds(off, length, src.length);
    ...
    FloatBuffer putArray(int index, float[] src, int off, int length) {
        int i = 0;
        while (i < length) {
     ...
     * @throws ReadOnlyBufferException if this buffer is read-only
     */
    public FloatBuffer put(int index, FloatBuffer src, int off, int length) {
        checkWritable();
        checkIndex(index, length);
    ...
    /** The unchecked copy behind {@link #put(int, FloatBuffer, int, int)}. */
    void putBuffer(int index, FloatBuffer src, int off, int length) {
        int i = 0;
        while (i < length) {
     ...
     *
     * <p><strong>Declared {@code Object} rather than {@code float[]}</strong>, unlike the JDK: our
     ...
     *
     * @return 2, because a float is four bytes wide
     */
    int scaleShifts() {
        return 2;
    }
     ...
     * @return {@code true} if {@code other} is a {@code FloatBuffer} with the same
     *         remaining contents
     */
    public boolean equals(Object other) {
        boolean same = false;
        if (other instanceof FloatBuffer) {
            FloatBuffer that = (FloatBuffer) other;
     ...
     * {@code float} terminates and {@code compareTo} stays consistent with itself.
     *
     ...
     *
     * @return a string of the form {@code FloatBuffer[pos=.. lim=.. cap=..]}
     */
    public String toString() {
        return "FloatBuffer[pos=" + position() + " lim=" + limit() + " cap=" + capacity() + "]";
    }
}

/**
 * The heap implementation of {@link FloatBuffer}. Top-level and package-private rather than
...
class HeapFloatBuffer extends FloatBuffer {
    ...
    HeapFloatBuffer(float[] hb, int offset, int capacity) {
    ...
    public FloatBuffer slice() {
        HeapFloatBuffer b = new HeapFloatBuffer(hb, offset + position(), remaining());
    ...
    public FloatBuffer slice(int index, int length) {
        checkIndex(index, length);
        HeapFloatBuffer b = new HeapFloatBuffer(hb, offset + index, length);
        b.isReadOnly = isReadOnly;
        return b;
    }

    public FloatBuffer duplicate() {
        HeapFloatBuffer b = new HeapFloatBuffer(hb, offset, capacity());
        b.position(position());
        b.limit(limit());
        b.isReadOnly = isReadOnly;
        return b;
    ...
    public FloatBuffer asReadOnlyBuffer() {
        HeapFloatBuffer b = (HeapFloatBuffer) duplicate();
        b.isReadOnly = true;
        return b;
    }
