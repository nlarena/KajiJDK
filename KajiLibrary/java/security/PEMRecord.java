...
// A PEM block exactly as it came out of the file: its label, its undecoded base64 body, and
// whatever was written before the "-----BEGIN".
//
// ===============================================================================================
...
// `javax.crypto.EncryptedPrivateKeyInfo`, which reads its own DER. With that, and with the blocks
// of unknown label --which come back as this record-- most of the API is really met; what needs a
...
//
// Like every `record` with an array component, `equals` compares `leadingData` **by reference**,
// not by content, and neither the constructor nor {@link #leadingData()} copies it. It is left that
// way because it is exactly what the JDK does: changing it would give a class with different
// semantics under the same name, which is worse than the surprise.
public record PEMRecord(String type, String content, byte[] leadingData) implements DEREncodable {
    ...
    /**
     * `leadingData` stays {@code null}, which is not the same as an empty array: it means "there
     * was nothing before the block", not "there were zero bytes".
     */
     ...
     *
     * <p>There is always at least one line of content, even when `content` is empty: a block with
     * no line in the middle is not what any other tool produces.
     *
