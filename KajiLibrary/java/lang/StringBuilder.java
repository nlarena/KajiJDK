...
/**
 * KajiLibrary's java.lang.StringBuilder -- the mutable character buffer the compiler lowers
 * string concatenation to ({@code a + b} becomes
 * {@code new StringBuilder().append(a).append(b).toString()}).
 *
 * <p>The whole class is a {@code char[]} and a count, and every method is arithmetic on those
 * two. The one thing plain bytecode cannot do is turn the array into a {@code String}, so
 * {@link #toString()} goes through {@code String.valueOf(char[], int, int)} and nothing else
 * here touches the VM.
 *
 * <p><strong>Why this is not shaped like the JDK's.</strong> The JDK puts the implementation in
 * a package-private {@code AbstractStringBuilder} that both this class and {@link StringBuffer}
 * extend. That shape depends on a piece of compiler machinery we do not have: when a public
 * class inherits public methods from a package-private superclass, {@code javac} synthesizes a
 * forwarder for each one, because a caller outside the package cannot NAME the superclass and so
 * the resolved method would fail the access check (JVMS 5.4.4). Our {@code javac} does not
 * synthesize them (finding #268), so with the JDK's shape {@code sb.length()} would resolve to a
 * method no caller outside {@code java.lang} is allowed to reach.
 *
 * <p>So the implementation lives HERE, in full, and {@link StringBuffer} holds one of these and
 * forwards to it under a lock. That keeps the property that mattered -- one implementation, so
 * the two classes cannot drift apart -- and costs {@code StringBuffer} one indirection, which is
 * the right place to put the cost: it is the legacy synchronized one, and this is the class the
 * compiler emits into every concatenation.
 */
public final class StringBuilder implements CharSequence, Appendable, Serializable,
     ...
     * @throws StringIndexOutOfBoundsException if any index is out of range
     */
    public StringBuilder insert(int index, char[] str, int strOffset, int len) {
     ...
     */
    public StringBuilder repeat(int codePoint, int times) {
        if (times < 0) {
     ...
     * NOT override {@code equals} -- a mutable object cannot have a stable hash code, so it would
     * be a trap in every hash-based collection.
     *
