...
final class IsoField implements TemporalField {
    ...
    // The four things `TemporalField` asks for to describe the field: what it counts, within what,
    // which values it allows, and how it is adjusted. The four ISO fields are derived from the date,
    // so their units come from `ChronoUnit` and from the two this same file defines.

    public TemporalUnit getBaseUnit() {
        if (this.kind == 0) {
            return ChronoUnit.DAYS;                      // day of the quarter
        }
        if (this.kind == 1) {
            return IsoFields.QUARTER_YEARS;              // quarter of the year
        }
        if (this.kind == 2) {
            return ChronoUnit.WEEKS;                     // week of the week-based year
        }
        return IsoFields.WEEK_BASED_YEARS;               // week-based year
    }
    ...
    public ValueRange range() {
        if (this.kind == 0) {
            // A quarter has between 90 and 92 days: a leap year's first has 91.
        ...
        if (this.kind == 2) {
            // An ISO year has 52 weeks, and 53 in the long years.
    ...
    public <R extends Temporal> R adjustInto(R temporal, long newValue) {
        // It adjusts **by difference**: how far to move in the base unit is computed and added.
        // Setting the field directly would call for rebuilding the date from the ISO week calendar,
        // which is a separate sum; moving by exactly the right amount gives the same result.
...
final class IsoUnit implements TemporalUnit {
    ...
    /**
     * How long it lasts, **estimated**: a week-based year is the mean year, a quarter its fourth
     * part.
     *
     * <p>Both are estimates and `isDurationEstimated` says so: an ISO year has 52 or 53 weeks
     * depending on which it is, and quarters have 90, 91 or 92 days.
     */
    public java.time.Duration getDuration() {
        if (this.kind == 0) {
            return java.time.Duration.ofSeconds(31556952L);        // week-based year
        }
        return java.time.Duration.ofSeconds(31556952L / 4L);       // quarter
    }

    /** It returns `temporal` plus `amount` of this unit. */
    public <R extends Temporal> R addTo(R temporal, long amount) {
        if (this.kind == 0) {
            // Adding week-based years: the week-based-year field is moved, which is the only thing
            // that keeps the week and the weekday. Adding 52 weeks will not do -- it drifts in the
            // years with 53.
            long current = IsoFields.WEEK_BASED_YEAR.getFrom(temporal);
            return (R) IsoFields.WEEK_BASED_YEAR.adjustInto(temporal, current + amount);
        }
        return (R) temporal.plus(amount * 3L, ChronoUnit.MONTHS);   // a quarter is three months
    }
