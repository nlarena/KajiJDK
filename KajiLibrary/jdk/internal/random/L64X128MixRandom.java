 ...
 *
 * <p>Note that the LCG's ADDEND {@code a} is per-instance and odd. That is what makes the family
 * splittable: two instances with different addends walk different LCG orbits, so their streams are
 * independent without needing to coordinate.
 *
 * @implNote CAREFUL: the xoroshiro128 used here is the "v1_0" variant, with rotations (24, 16, 37)
 *           — NOT the (49, 21, 28) of Xoroshiro128PlusPlus. Same family, different tuning; they are
 *           not interchangeable.
 *
 * @implNote An INTERNAL class. A KajiLibrary subset: the JDK's implements the nested
 *           SplittableGenerator interface (finding #101), so {@code split()} is omitted.
 */
